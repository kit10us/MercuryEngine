...
#pragma region Create the depth-stencil buffer...
...

	{
		...
		result = dxDevice->CreateDepthStencilView( depthStencilBuffer, nullptr, &depthStencilView );
		assert( ! FAILED( result ) );
...
#pragma region Create vertex shader.
...

	{
		...
		result = D3DCompileFromFile( _T("test2D.hlsl"), shaderMacros, D3D_COMPILE_STANDARD_FILE_INCLUDE, "vs_main", "vs_4_0", flags1, flags2, &vertexShaderBuffer, &errorBlob );
		if ( FAILED( result ) )
		{
...
#pragma region Create pixel shader...
...

	{
		...
		unsigned int flags2 = 0; // Only used for effect compilation.
		result = D3DCompileFromFile( _T("test2D.hlsl"), shaderMacros, D3D_COMPILE_STANDARD_FILE_INCLUDE, "ps_main", "ps_4_0", flags1, flags2, &pixelShaderBuffer, &errorBlob );
		if ( FAILED( result ) )
		{
			OutputDebugStringA( (char*)errorBlob->GetBufferPointer() );
			assert( ! FAILED( result ) );
...
#pragma region Setup shader constant transforms for CPU memory (non-GPU/uploaded to GPU)...
	...
	} vertexShaderConstants;
	{
		...
		vertexShaderConstants.viewMatrix = XMMatrixIdentity();
		vertexShaderConstants.projectionMatrix = XMMatrixOrthographicOffCenterLH( 0.0f, viewport.Width, viewport.Height, 0, 1.0f, 1000.0f );
		//vertexShaderConstants.projectionMatrix = XMMatrixOrthographicLH( viewport.Width, viewport.Height, 1.0f, 1000.0f );
		//vertexShaderConstants.projectionMatrix = XMMatrixTranspose( vertexShaderConstants.projectionMatrix );
	}
#pragma endregion
...
#pragma endregion
	...
	} vbRaw [ 4 ] =
	{
		...
		{ viewport.Width, 0, depth, 0.0f, 1.0f, 0.0f, 1.0f },
		{ 0, viewport.Height, depth, 0.0f, 0.0f, 1.0f, 1.0f },
		{ viewport.Width, viewport.Height, depth, 1.0f, 1.0f, 1.0f, 1.0f }
	};

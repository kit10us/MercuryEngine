	...
	while( forever )
	{
		...
		for( int d = 0; d < totalD; ++d )
		{
			for( int c = 0; c < totalC; ++c )
			{
				for( int r = 0; r < totalR; ++r )
				{
					int i = r + c * totalR + d * (totalR * totalC);
					instanceData[ i ].matrix = DirectX::XMMatrixTranslation( -((size + 1) * totalR * 0.5f) + (size + 1) * r, -((size + 1) * totalC * 0.5f) + (size + 1) * c, -((size + 1) * totalD * 0.5f) + (size + 1) * d );

					int axisIndex = i % 3;
					XMVECTOR axis = XMVectorSet( axisIndex == 0, axisIndex == 1, axisIndex == 2, 1.0f );
					instanceData[ i ].matrix *= XMMatrixRotationAxis( axis, rotationInRadians );
				}
			}
		}

		{
			...
			result = dxContext->Map( instanceBuffer, 0, D3D11_MAP_WRITE_DISCARD, mapFlags, &mappedResource );
			assert( !FAILED( result ) );

			memcpy( mappedResource.pData, &instanceData[0], sizeof( InstanceData ) * totalInstances );
			dxContext->Unmap( instanceBuffer, 0 );

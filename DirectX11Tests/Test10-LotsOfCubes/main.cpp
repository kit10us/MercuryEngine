...
#pragma region Create pixel shader...
	...
	CComPtr< ID3D10Blob > pixelShaderBuffer;
	{
		...
		unsigned int flags2 = 0; // Only used for effect compilation.
		result = D3DCompileFromFile( _T("test3D.hlsl"), shaderMacros, D3D_COMPILE_STANDARD_FILE_INCLUDE, "ps_main", "ps_4_0", flags1, flags2, &pixelShaderBuffer, &errorBlob );
		if ( FAILED( result ) )
		{
			OutputDebugStringA( (char*)errorBlob->GetBufferPointer() );
			assert( ! FAILED( result ) );
...
#pragma region Per-object matrices...
	...
	for( int d = 0; d < totalD; ++d )
	{
		for( int c = 0; c < totalC; ++c )
		{
			...
			{
				matrices.push_back( DirectX::XMMatrixTranslation( -((size + 1) * totalR * 0.5f) + (size + 1 ) * r, -((size + 1) * totalC * 0.5f) + (size + 1) * c, -((size + 1) * totalD * 0.5f) + (size + 1) * d ) );
			}
		}
	}
#pragma endregion
	...
	while( forever )
	{
		...
		for( auto matrix : matrices )
		{
			// Update matrices...
			{
				...
				vertexShaderConstants.worldMatrix = rotationMatrix * matrix;

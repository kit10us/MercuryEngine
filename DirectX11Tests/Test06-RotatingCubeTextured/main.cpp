...
#pragma region Create vertex shader.
	...
	CComPtr< ID3D11ShaderReflection > vertexShaderReflection;
	{
		...
		result = D3DCompileFromFile( _T("test3DTextured.shader"), shaderMacros, D3D_COMPILE_STANDARD_FILE_INCLUDE, "vs_main", "vs_4_0", flags1, flags2, &vertexShaderBuffer, &errorBlob );
		if ( FAILED( result ) )
		{
...
#pragma region Create pixel shader...
	...
	CComPtr< ID3D10Blob > pixelShaderBuffer;
	{
		...
		unsigned int flags2 = 0; // Only used for effect compilation.
		result = D3DCompileFromFile( _T("test3DTextured.shader"), shaderMacros, D3D_COMPILE_STANDARD_FILE_INCLUDE, "ps_main", "ps_4_0", flags1, flags2, &pixelShaderBuffer, &errorBlob );
		if ( FAILED( result ) )
		{
			OutputDebugStringA( (char*)errorBlob->GetBufferPointer() );
			assert( ! FAILED( result ) );
...
#pragma region Create texture...
	...
	CComPtr< ID3D11SamplerState > samplers;
	{
		result = DirectX::CreateDDSTextureFromFile( dxDevice, L"borgcube.dds", (ID3D11Resource**)&texture, &textureView );
		assert( ! FAILED( result ) );
		...
		result = dxDevice->CreateSamplerState( &samplerDesc, &samplers );
	}
...
#pragma region The main loop...
	...
	while( forever )
	{
		...
		dxContext->PSSetShaderResources( 0, 1, &textureView.p );
		dxContext->PSSetSamplers( 0, 1, &samplers.p );

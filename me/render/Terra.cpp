// Copyright (c) 2002 - 2010 Kit10 Studios LLC
// All Rights Reserved
...
Terra::Parameters::Parameters( const unify::Size< float > size, const unify::RowColumn< unsigned int > faces, const float constant, const unify::TexArea texArea, Effect::ptr effect )
{
	Set( "size", size );
	Set( "effect", effect ); 
...
void Terra::Parameters::SetEffect( Effect::ptr effect )
{
	Set( "effect", effect ); 
}
...
Terra::Terra( IRenderer * renderer )
: Mesh( renderer )
, m_depth{}
, m_pointCount( 0, 0 )
{
...
Terra::Terra( IRenderer * renderer, unify::Parameters & parameters )
: Terra( renderer )
{
...
void Terra::CreateFromParameters( unify::Parameters & parameters )
{
	...
	for ( unsigned int c = 0; c < faces.column; ++c )
	{
		for ( unsigned int r = 0; r < faces.row; ++r )
		{
			...
			unsigned int firstIndex = (unsigned int)(IndicesPerRow * c + r * IndicesPerCell);

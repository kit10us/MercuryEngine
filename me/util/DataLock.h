...
namespace me::util
{
	...
	T* DataLock::GetData()
	{
		...
		{
			throw unify::Exception("Attempted to access " + Cast<std::string>(m_type) + " for " + Cast<std::string>(DataLockAccess::ReadWrite) + " access!");
		}
		return (T*)m_data;
	}
...
namespace me
{
	template<>
	inline
	util::DataLockAccess::TYPE Cast(const std::string access)
	{
		using unify::String::StringIs;

		if (StringIs(access, "None"))
		{
			return util::DataLockAccess::None;
		}
		else if (StringIs(access, "Readonly"))
		{
			return util::DataLockAccess::Readonly;
		}
		else if (StringIs(access, "Writeonly"))
		{
			return util::DataLockAccess::Writeonly;
		}
		else if (StringIs(access, "ReadWrite"))
		{
			return util::DataLockAccess::ReadWrite;
		}
	...
	template<>
	inline
	std::string Cast(const util::DataLockAccess::TYPE access)
	{
		switch (access)
		{
		case util::DataLockAccess::None:
			return "None";

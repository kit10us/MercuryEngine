...
void ASEDocument::Load( const unify::Path & filePath )
{
	...
	qxml::Element * pParent = AddElement( new qxml::Element( "Root", qxml::Element::NodeType::Element ) , "Root" );
	...
	while( !stream.EndOfStream() )
	{
		...
		else if( sData[ 0 ] == '*' ) 
		{
			...
			qxml::Element * pElement = new qxml::Element( sName, qxml::Element::NodeType::Element );

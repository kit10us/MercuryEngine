...
namespace dxi
{
    ...
    namespace scene
    {
		...
		Object::shared_ptr MakeObject( geo::Geometry * geometry );

	    class Scene;
	    class ScenePathSource : public pathing::PathSystem::PathSource
	    {
		    const Scene * m_scene;
	    public:
			ScenePathSource( const Scene * scene );
	    ...
	    public:
		    ...
		    pathing::PathSystem & GetPathSystem();
		    const pathing::PathSystem & GetPathSystem() const;
            ...
            void SetDefaultCullMode( DWORD cullMode );
		    
            void SetSize( const unify::Size< int > & size );
            const unify::Size< int > & GetSize() const;
		    
            void SetPosition( const unify::V2< int > & position );
            const unify::V2< int > & GetPosition() const;
		    ...
		    controllers::ControllerFactory * GetControllerFactory();
		    const controllers::ControllerFactory * GetControllerFactory() const;
	    ...
	    private:
		    ...
		    pathing::PathSystem m_pathSystem;
		    std::shared_ptr< ScenePathSource > m_scenePathSource;
		    controllers::ControllerFactory m_controllerFactory;
		    ...
		    DWORD m_defaultCullMode;

		    // Viewport...
		    unify::Size< int > m_size;
		    unify::V2< int > m_position;
		    unify::MinMax< float > m_z;

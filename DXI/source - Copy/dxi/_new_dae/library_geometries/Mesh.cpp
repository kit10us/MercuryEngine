...
: DocumentNode( document, node )
{
	for ( const qxml::Element * childNode = node->GetFirstChild(); childNode; childNode = childNode->GetNext() )
	{
		...
		{
			boost::shared_ptr< Source > source( new Source( document, childNode ) );
		...
		else if ( childNode->IsTagName( "polylist" ) )
		{
			...
			boost::shared_ptr< Polylist > polylist( new Polylist( document, childNode ) );
...
const std::vector< boost::shared_ptr< Source > > & Mesh::GetSource() const
{
	return m_source;
}

boost::shared_ptr< Source > Mesh::GetSource( const std::string & name ) const
{
	std::string realName;
...
{
	for ( std::vector< boost::shared_ptr< Polylist > >::const_iterator itr = m_polylist.begin(), end = m_polylist.end(); itr != end; ++itr )
	{

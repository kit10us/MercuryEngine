...
, m_name( node->GetStringAttributeElse( "name", std::string() ) )
{
	for ( const qxml::Element * childNode = node->GetFirstChild(); childNode; childNode = childNode->GetNext() )
	{
		...
		{
			m_input.push_back( boost::shared_ptr< Input_Unshared >( new Input_Unshared( document, childNode ) ) );
		}
	}
}
...
const std::vector< boost::shared_ptr< Input_Unshared > > & Vertices::GetInput() const
{
	return m_input;
}

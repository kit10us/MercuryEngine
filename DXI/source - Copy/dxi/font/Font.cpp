...
Font::Font( const unify::Path & filePath, Managers managers, animation::SpriteManager::shared_ptr spriteManager )
{
	CreateFromFile( filePath, managers, spriteManager );
}
...
void Font::CreateFromFile( const unify::Path & filePath, Managers managers, animation::SpriteManager::shared_ptr spriteManager )
{
	...
	while( leaf )
	{
        if ( leaf->IsTagName( "effect" ) )
        {
			...
			m_effect = loader::EffectLoader( name, leaf, managers );
		...
		else if( leaf->IsTagName( "character" ) )
		{
			...
			if( leaf->HasAttributes( "texture" ) )
			{
				...
				if( leaf->HasAttributes( "group,sequence" ) )
				{
					...
					if( ! managers.GetTextureManager()->Exists( textureName ) )
					{
						throw unify::Exception( "Specfied texture for character font does not exist!" );
					}

					Texture::shared_ptr texture( managers.GetTextureManager()->Find( textureName ) );
			...
			else if( leaf->HasAttributes( "geometry" ) )
			{
				...
				geo::Geometry::shared_ptr geometry( managers.GetGeometryManager()->LoadFromFile( geometryName, geometryPath )  );

...
public:
	scene::Object::shared_ptr cameraObject;
	scene::Camera::shared_ptr camera;

	geo::Geometry::shared_ptr cube;

	geo::Geometry::shared_ptr group;

	scene::Object::shared_ptr groupObject;
	...
	void Startup()
	{
		...
		cameraObject.reset( new scene::Object ); // Necessary to store the camera object -somewhere- outside of the camera, as is weak_ptr in camera.
		camera.reset( new scene::Camera( cameraObject ) );
		camera->SetProjection( unify::Matrix::MatrixPerspectiveFovLH( D3DX_PI / 4.0f, GetOS().GetResolution().AspectRatioHW(), 1, 1000 ) );
		...
		cube.reset( new geo::Mesh( cubeParameters ) );
		...
		groupObject.reset( new scene::Object( geo::Geometry::shared_ptr( geometryGroup ) ) );
	}
	...
	void Render()
	{
		...
		ri.SetFinalMatrix( camera->GetMatrix() );
		//groupObject->Render( ri );
		//cube->Render( ri, nullptr );
		//group->Render( ri, nullptr );
		ri.SetOption( RenderOption::NoFrame, true );
		groupObject->Render( ri );

...
bool Stream::Write( const void * pSrc, unsigned int uLength )
{
	pSrc;
	uLength;
	// Fail by default.
...
bool Stream::WritePack( const void * pSrc )
{
	pSrc;
	// Fail by default.
	return false;
...
void Stream::SeFlags( unsigned int uFlags )
{
	m_uFlags = uFlags;
}

// Returns the currently set stream flags.
unsigned int Stream::GeFlags()
{
	return m_uFlags;
}

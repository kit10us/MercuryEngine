...
function CreateSand( terra_size )
	...
	local vs = VertexShader( "texturemix_ambient.xml")
	local ps = PixelShader( "texturemix_ambient.xml" )
	local t1 = Texture( "sand", "sand_tile.bmp" )
	local t2 = Texture( "grass", "grass.bmp" )
	local t3 = Texture( "sand_mix", "sand_mix.png" )
...
function CreateSidewalk( terra_size )
	...
	local vs = VertexShader( "texturemix_ambient.xml")
	local ps = PixelShader( "texturemix_ambient.xml" )
	local t1 = Texture( "wall", "wallx.dds" )
	local t2 = Texture( "leafsolid", "leafsolid.bmp" )
	local t3 = Texture( "sidewalk_mix", "river27b_h.bmp" )
	local effect = Effect( vs, ps, t1, t2, t3 )
	parameters:SetEffect( effect )
